...
## Outside resources
In my `Exceptions/Handler.php`, I used code provided from https://stackoverflow.com/questions/30276325/laravel-5-how-do-i-handle-methodnotallowedhttpexception to help set up a redirect to home when a MethodNotAllowedHTTPException is thrown. It triggers when a user tries to access post routes via the address bar.
...
+ Added extra validation in the controller to prevent users unauthorized access by using the address bar. (for instance, you can edit mod id 1 at */1/edit* when mod id 1 is yours, but typing in */2/edit* in the address bar when mod id 2 is not yours will not allow access to editing mod id 2 - this was an issue up until my conditionals)
